...
import { kwHit, rmDia } from './intent';
...
export const STATIC_SLIDES: CatalogEntry[] = fromJSON<CatalogEntry[]>('static_slides', MIN_STATIC_SLIDES) || [
  ...
  {
    keywords: ['an ninh', 'bảo vệ', 'camera', 'an toàn không', 'trộm cắp', 'trộm cướp', 'mất trộm', 'an toàn ban đêm', 'compound', 'khép kín', 'pccc', 'phòng cháy', 'cháy nổ', 'thoát hiểm'],
    slide: {
...
export function matchStaticSlide(message: string, phase: 'combo' | 'general'): CatalogSlide | null {
  ...
  const noD = rmDia(msg);
  // kwHit = so khớp có biên từ + hạn chế khớp bỏ-dấu cho từ ngắn (lib/intent.ts).
  const hit = (kw: string) => kwHit(msg, noD, kw);
